...
import com.bulletphysics.demos.input.InputKey;
import com.bulletphysics.demos.input.Keyboard;
import com.bulletphysics.demos.input.Mouse;
import com.bulletphysics.demos.input.MouseButton;
 ...
 * @author jezek2
 */
public abstract  class DemoApplication    implements GLEventListener{
	...
	public void init(GLAutoDrawable arg0) {
		((JoglGL)gl).init(arg0.getGL().getGL2());
	 ...
	 public void reshape(GLAutoDrawable drawable, int x, int y, int width, int height) {
	     ((JoglGL) gl).init(drawable.getGL().getGL2());
			...
			case 1 -> {
				if (state == 0) {
					...
					if (dynamicsWorld != null) {
						...
						dynamicsWorld.rayTest(cameraPosition, rayTo, rayCallback, RayResultCallback.RAY_EPSILON);
